...
- Planner Engine (TypeScript)
  - Implement planJob(context: PlannerContext): PlannerDecision in src/lib/ffmpeg-plan.ts. Steps: 1. Validate preset availability against capabilities + container rules. 2. Determine stream strategy (full remux, copy video + transcode audio, re-encode video) based on probe summary and
    preset requirements. 3. Construct ffmpeg argument array (maps, codec flags, bitrate/profile, -movflags +faststart, color metadata
    passthrough). 4. Decide subtitle handling (convert/burn/drop) per preset. 5. Emit warnings/notes (e.g., tier adjustments, missing HDR metadata).
...
- **WebM | VP9 + Opus**
- **WebM | AV1 + Opus** _(Experimental; slow; hide if encoder missing)_
- **MOV | ProRes 422 HQ + PCM** (editing/archival)
- **MKV | Passthrough** (copy streams when allowed)
- **GIF** _(guard length/size; warn >20s)_
