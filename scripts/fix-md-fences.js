...
async function fixFile(file) {
  ...
  // Replace any non-blank line followed by newline and ``` with a blank line in between
  txt = txt.replace(/([^\n])\n(```[\s\S]*?```)/g, (m, before, rest) => {
    // only insert if there's not already a blank line (i.e., previous char not a newline)
    return before + '\n\n' + rest;
  });

  // Ensure blank line after closing fence: replace closing ``` followed by non-blank line without blank
  txt = txt.replace(/(```[\s\S]*?```)\n([^\n])/g, (m, fence, after) => {
    return fence + '\n\n' + after;
  });
